...
| passengersCount           | integer | Total number of passengers to transport (adults and children). Must be between 1 and 7 if includeMultipleVehicle is set to `false` or not provided. In case that includeMultipleVehicle is set to `true` this parameter must be between 1 and 99.                                                                     |
...
| includeMultipleVehicle    | boolean | Optional. Defaults to `false`. When set to `true`, if single vehicle options cannot be found we will try to provide trip options with multiple vehicles.                                                                                                                                                              |
...
| vehicle                 | object - [Vehicle](#vehicle)                             | Details about the vehicle. Set to `null` in case of multi-vehicle trip options.                                                                                          |
| vehicles                | list of [Vehicle](#vehicle)                              | Details about vehicles. Defined only if includeMultipleVehicle set to `true`.                                                                                            |
