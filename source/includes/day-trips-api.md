...
1.  Call [/partners/v1/daytrips/country/{countryIsoCode}](#browse-locations) to discover all available departure locations within a country.
2.  Present the locations to the user. Once a location is chosen, call [/partners/v1/daytrips/location/{locationId}](#day-trips-by-location) to fetch all available day trip packages from that city.
3.  After the user selects a day trip, call [/partners/v1/daytrips/book](#book-day-trip) with the `dayTripId` and passenger details to create a booking.
...
1.  Call [/partners/v1/daytrips/search](#search-day-trips) using the user's latitude and longitude. The API automatically finds the nearest departure location and returns all associated day trips.
2.  After the user selects a day trip, call [/partners/v1/daytrips/book](#book-day-trip) with the `dayTripId` and passenger details.
